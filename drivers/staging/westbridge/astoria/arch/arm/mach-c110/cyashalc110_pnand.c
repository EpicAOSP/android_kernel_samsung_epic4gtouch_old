...
void cy_as_hal_c110_hardware_deinit(cy_as_c110_dev_kernel *dev_p)
{
	...
	if (dev_p->m_phy_addr_base != 0)
		release_mem_region((unsigned long)dev_p->m_phy_addr_base, SZ_16K);
 ...
 **/
static void cy_as_hal_pNand_lbd_read(cy_as_hal_device_tag tag, u32 row_addr, u16 count, void *buff)
{
	cy_as_c110_dev_kernel *dev_p = (cy_as_c110_dev_kernel *)tag ;
    uint16_t w16cnt;
    uint16_t *ptr16;
 ...
 **/
static void cy_as_hal_pNand_lbd_write(cy_as_hal_device_tag tag, u32 row_addr, u16 count, void *buff)
{
	cy_as_c110_dev_kernel *dev_p = (cy_as_c110_dev_kernel *)tag ;
    uint16_t w16cnt;
    uint32_t *ptr16;
...
#ifdef DBGPRN_ENABLED
			DBGPRN("<1>cy_service_e_p_dma_read_request cb: xfer_sz:%d ep:%d ptr:0x%x\n",
...
/*	write_addr =  (void *)  (dev_p->m_vma_addr_base + CYAS_DEV_CALC_EP_ADDR (ep)) ;  */
	...
	if (size == 512) {
		...
		dma_unmap_single(NULL,dma_source_addr,size,DMA_TO_DEVICE);
	...
	writel(CYASHAL_PNAND_CONFIG_WRITE_SET, vma_addr_base );
/*
    	while (w16cnt--) {
	...
	if (size & 3) {
		...
		t16=(uint16_t *)dptr;
	      ...
	      if(size &1)
               IOWR8(vma_addr_data,  *(uint8_t *)t16);
	 }
	#endif
	}
...
static void cy_handle_d_r_q_interrupt(cy_as_c110_dev_kernel *dev_p, uint16_t read_val)
{
	...
	if (v == 0) {
#ifndef WESTBRIDGE_NDEBUG
		printk("<l>stray drq interrupt detected: read_val = 0x%x", read_val) ;
		printk("<l>P0_DRQ=0x%x", cy_as_hal_read_register(
						...
						uint32_t size, uint16_t maxsize)
{
	...
	if (end_points[ep].sg_list_enabled) {
		...
		DBGPRN("<l>cyasc110hal:%s: EP:%d, buf:%p, buf_va:%p, req_sz:%d, maxsz:%d\n",
					...
					uint32_t size, uint16_t maxsize)
{
	...
	if (end_points[ep].sg_list_enabled) {
		...
		DBGPRN("<1>cyasc110hal:DMA_setup_read sg_list EP:%d, buf:%p, buf_va:%p, req_sz:%d, maxsz:%d\n",
...
void cy_as_hal_pll_lock_loss_handler(cy_as_hal_device_tag tag)
{
	...
	cy_as_hal_print_message("<l>please check the input voltage levels");
...
cy_bool cy_as_hal_sleep_on(cy_as_hal_sleep_channel *channel, uint32_t ms)
{
	...
	wait_event_interruptible_timeout(channel->wq, cy_as_hal_sleep_condition, ((ms * HZ)/1000)) ;
	return cy_true ;
}
				...
				cy_as_hal_device_tag *tag, cy_bool debug)
{
	...
	cy_as_hal_write_register((cy_as_hal_device_tag)dev_p, CY_AS_MEM_PNAND_CFG,0x0);
