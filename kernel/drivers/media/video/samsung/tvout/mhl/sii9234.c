...
struct sii9234_data {
	...
	struct msc_packet		msc_queue[MSC_MAX_QUEUE];
	int				msc_front;
	int				msc_rear;
					...
					struct msc_packet *msc_pkt)
{
	...
	case CBUS_GET_DDC_ERR_CODE:
		...
		break;
	default:
		pr_info("%s() invalid msc command(%d)\n",
...
void sii9234_process_msc_work(struct work_struct *work)
{
	...
	if (!sii9234->msc_queue[sii9234->msc_front].sent) {
		...
		sii9234->msc_queue[sii9234->msc_front].sent = true;
...
exit:
	...
	sii9234->msc_front = (sii9234->msc_front + 1) % MSC_MAX_QUEUE;
	if (sii9234->msc_front > sii9234->msc_rear ||
		(sii9234->msc_front == 0 &&
			sii9234->msc_rear == MSC_MAX_QUEUE-1))
		sii9234->msc_front--;

	/* check if next work is in queue */
	if (!sii9234->msc_queue[sii9234->msc_front].sent) {
		pr_info("sii9234: %s() msc work schedule\n", __func__);
		...
		u8 offset, u8 data_1, u8 data_2)
{
	...
	next_idx = (sii9234->msc_rear + 1) % MSC_MAX_QUEUE;

	if (next_idx == sii9234->msc_front) {
		pr_info("sii9234: queue full\n");
		return;
	}
	...
	sii9234->msc_queue[sii9234->msc_rear].sent = false;

	sii9234->msc_rear = next_idx;
...
/* this point is very importand before megsure RGND impedance*/
	...
	if (sii9234->msc_ready) {
		...
		printk(KERN_ERR"%s() msc not ready : ret=%d, msc_ready=%d\n",
			__func__, ret, sii9234->msc_ready);
	}
	mutex_lock(&sii9234->lock);
...
static ssize_t sysfs_sii9234_ddc_i2c_num_show(struct class *class,
	struct class_attribute *attr, char *buf)
{
	int size;
	struct sii9234_data *sii9234 = dev_get_drvdata(sii9244_mhldev);
	...
	pr_info("%s() ddc_i2c_num : %d\n",
		__func__, sii9234->pdata->ddc_i2c_num);
	size = sprintf(buf, "DDC %d\n", sii9234->pdata->ddc_i2c_num);

	return size;
}

static CLASS_ATTR(ddc_i2c_num, 0664 , sysfs_sii9234_ddc_i2c_num_show, NULL);
					      ...
					      const struct i2c_device_id *id)
{
	...
	pr_debug("sii9234: wake lock is initialized.\n");
#endif
